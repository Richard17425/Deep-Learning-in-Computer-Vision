...
### **隐藏层**激活函数的选择
![](images/22.png)   
**ReLU函数**如今在隐藏层中应用最广泛，一个是因为它的计算速度更快，只需要计算最大值，第二个也是最主要的原因是ReLU函数只有左边一边是(完全)平坦的，而sigmoid函数两边都是平坦的，这就导致后者在运用梯度下降算法训练神经网络时会很缓慢(梯度下降算法优化成本函数J(W,b)并不会优化激活函数)，减小了学习速度。
